...
public class OnSelectionMenu extends ListenerAdapter {
    ...
    public void onModalInteraction(@NotNull ModalInteractionEvent event) {
        if (event.getModalId().startsWith("register")) {
            String memberId = event.getModalId().split(":")[1];
            String roleName = event.getValue("role").getAsString();
            RPGUser.newInfo(event.getUser().getIdLong());
            try {
                event.getGuild().addRoleToMember(memberId, event.getGuild().getRolesByName(roleName, true).get(0)).queue();
            } catch (Exception e) {
                event.reply("An unknown error occurred. The role name is either invalid or there is no such role.").setEphemeral(true).queue();
                return;
            }
            event.reply("Successfully registered the user's account!").queue();
        }
        ...
        if (event.getModalId().equals("loan")) {
            TextChannel loanChannel = event.getGuild().getTextChannelsByName("loans", true).get(0);
            Role adminA = event.getGuild().getRoleById(Config.get("admina"));
            Role adminB = event.getGuild().getRoleById(Config.get("adminb"));
            Role adminC = event.getGuild().getRoleById(Config.get("adminc"));
            EmbedBuilder embedBuilder = new EmbedBuilder();
            embedBuilder.setTitle("New Loan Request");
            embedBuilder.setDescription("User: **" + event.getUser().getAsTag() + "**\n" +
                    "Amount: **$" + event.getValue("amount").getAsString() + "**\n" +
                    "Reason: ```java\n" +
                    event.getValue("message").getAsString() + "```");
            loanChannel.sendMessage(adminA.getAsMention() + " " + adminB.getAsMention() + " " + adminC.getAsMention()).setEmbeds(embedBuilder.build()).queue();
            event.reply("Successfully sent the request!").setEphemeral(true).queue();
        }
    }
}